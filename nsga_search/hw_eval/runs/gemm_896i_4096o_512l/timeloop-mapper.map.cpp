  ...
  Weights.Resize({ D5, G4, G3, D3s, E2, G1, E1s });
  Inputs.Resize({ G4, G3, E2, L2, G1, E1s, L0 });
  ...
  // DRAM tiles 
  Weights.AddTileLevel(D5*G4*G3*D3s*E2*G1*E1s*1, D5*G4*G3*D3s*E2*G1*E1s*1, 1, 1 * kBackingStoreLatency);
  Weights.BindCurrentTileLevel("DRAM", 1);
  Inputs.AddTileLevel(G4*G3*E2*L2*G1*E1s*L0*1, G4*G3*E2*L2*G1*E1s*L0*1, 1, 1 * kBackingStoreLatency);
              ...
              t_for(g1, 0, G1); {
                s_for(e1s, 0, E1s); {
                  ...
                  t_for(l0, 0, L0); {
                    ...
                    Weights[d5][g4][g3][d3s][e2][g1][e1s]; // read-only
                    Inputs[g4][g3][e2][l2][g1][e1s][l0]; // read-only
