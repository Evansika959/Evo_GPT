  ...
  static const int E5 = 4;
  static const int D5 = 88;
  ...
  Weights.Resize({ E5, D5, G4, G3, D3s, E2, G1, E1s });
  Inputs.Resize({ E5, G4, G3, E2, L2, G1, E1s, L0 });
  Outputs.Resize({ D5, G4, G3, D3s, L2, G1, L0 });
  ...
  // DRAM tiles 
  Weights.AddTileLevel(E5*D5*G4*G3*D3s*E2*G1*E1s*1, E5*D5*G4*G3*D3s*E2*G1*E1s*1, 1, 1 * kBackingStoreLatency);
  Weights.BindCurrentTileLevel("DRAM", 1);
  Inputs.AddTileLevel(E5*G4*G3*E2*L2*G1*E1s*L0*1, E5*G4*G3*E2*L2*G1*E1s*L0*1, 1, 1 * kBackingStoreLatency);
  Inputs.BindCurrentTileLevel("DRAM", 1);
  Outputs.AddTileLevel(D5*G4*G3*D3s*L2*G1*L0*1, D5*G4*G3*D3s*L2*G1*L0*1, 1, 1 * kBackingStoreLatency);
  ...
  t_for(e5, 0, E5); {
    t_for(d5, 0, D5); {
      ...
      // Scratchpad tiles 
      Weights.AddTileLevel(G4*G3*D3s*E2*G1*E1s*1, G4*G3*D3s*E2*G1*E1s*1, 1, 1 * kDRAMLatency);
      Weights.BindCurrentTileLevel("Scratchpad", 1);
      Inputs.AddTileLevel(G4*G3*E2*L2*G1*E1s*L0*1, G4*G3*E2*L2*G1*E1s*L0*1, 1, 1 * kDRAMLatency);
                ...
                t_for(g1, 0, G1); {
                  s_for(e1s, 0, E1s); {
                    ...
                    t_for(l0, 0, L0); {
                      ...
                      Weights[e5][d5][g4][g3][d3s][e2][g1][e1s]; // read-only
                      Inputs[e5][g4][g3][e2][l2][g1][e1s][l0]; // read-only
                      Outputs[d5][g4][g3][d3s][l2][g1][l0]; // read-write
