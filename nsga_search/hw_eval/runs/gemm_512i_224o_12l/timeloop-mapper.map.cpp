  ...
  static const int L0 = 12;

  Weights.Resize({ G5, G4, G3, D3s, D2, E2, G1, E1s });
  Inputs.Resize({ G5, G4, G3, E2, G1, E1s, L0 });
  Outputs.Resize({ G5, G4, G3, D3s, D2, G1, L0 });
  ...
  // DRAM tiles 
  Weights.AddTileLevel(G5*G4*G3*D3s*D2*E2*G1*E1s*1, G5*G4*G3*D3s*D2*E2*G1*E1s*1, 1, 1 * kBackingStoreLatency);
  Weights.BindCurrentTileLevel("DRAM", 1);
  Inputs.AddTileLevel(G5*G4*G3*E2*G1*E1s*L0*1, G5*G4*G3*E2*G1*E1s*L0*1, 1, 1 * kBackingStoreLatency);
  Inputs.BindCurrentTileLevel("DRAM", 1);
  Outputs.AddTileLevel(G5*G4*G3*D3s*D2*G1*L0*1, G5*G4*G3*D3s*D2*G1*L0*1, 1, 1 * kBackingStoreLatency);
  Outputs.BindCurrentTileLevel("DRAM", 1);

  t_for(g5, 0, G5); {

    // Scratchpad tiles 
    Weights.AddTileLevel(G4*G3*D3s*D2*E2*G1*E1s*1, G4*G3*D3s*D2*E2*G1*E1s*1, 1, 1 * kDRAMLatency);
    Weights.BindCurrentTileLevel("Scratchpad", 1);
    Inputs.AddTileLevel(G4*G3*E2*G1*E1s*L0*1, G4*G3*E2*G1*E1s*L0*1, 1, 1 * kDRAMLatency);
    ...
    t_for(g4, 0, G4); {
      ...
      t_for(g3, 0, G3); {
        s_for(d3s, 0, D3s); {
          ...
          Outputs.AddTileLevel(D2*G1*L0*1, D2*G1*L0*1, 1, 1 * kinter_PERows_spatialLatency);
              ...
              t_for(g1, 0, G1); {
                s_for(e1s, 0, E1s); {
                  ...
                  t_for(l0, 0, L0); {
                    ...
                    Weights[g5][g4][g3][d3s][d2][e2][g1][e1s]; // read-only
                    Inputs[g5][g4][g3][e2][g1][e1s][l0]; // read-only
                    Outputs[g5][g4][g3][d3s][d2][g1][l0]; // read-write
