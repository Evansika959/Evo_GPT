  ...
  static const int L5 = 2;
  static const int D5 = 192;
  static const int L4 = 2;
  static const int E4 = 3;
  ...
  Weights.Resize({ D5, E4, G3, D3s, E2, G1, E1s });
  Inputs.Resize({ L5, L4, E4, G3, E2, L2, G1, E1s, L0 });
  Outputs.Resize({ L5, D5, L4, G3, D3s, L2, G1, L0 });
  ...
  // DRAM tiles 
  Weights.AddTileLevel(D5*E4*G3*D3s*E2*G1*E1s*1, D5*E4*G3*D3s*E2*G1*E1s*1, 1, 1 * kBackingStoreLatency);
  Weights.BindCurrentTileLevel("DRAM", 1);
  Inputs.AddTileLevel(L5*L4*E4*G3*E2*L2*G1*E1s*L0*1, L5*L4*E4*G3*E2*L2*G1*E1s*L0*1, 1, 1 * kBackingStoreLatency);
  Inputs.BindCurrentTileLevel("DRAM", 1);
  Outputs.AddTileLevel(L5*D5*L4*G3*D3s*L2*G1*L0*1, L5*D5*L4*G3*D3s*L2*G1*L0*1, 1, 1 * kBackingStoreLatency);
  ...
  t_for(l5, 0, L5); {
    t_for(d5, 0, D5); {
      ...
      // Scratchpad tiles 
      Weights.AddTileLevel(E4*G3*D3s*E2*G1*E1s*1, E4*G3*D3s*E2*G1*E1s*1, 1, 1 * kDRAMLatency);
      Weights.BindCurrentTileLevel("Scratchpad", 1);
      Inputs.AddTileLevel(L4*E4*G3*E2*L2*G1*E1s*L0*1, L4*E4*G3*E2*L2*G1*E1s*L0*1, 1, 1 * kDRAMLatency);
        ...
        t_for(e4, 0, E4); {
          ...
          t_for(g3, 0, G3); {
            s_for(d3s, 0, D3s); {
              ...
              Outputs.AddTileLevel(L2*G1*L0*1, L2*G1*L0*1, 1, 1 * kinter_PERows_spatialLatency);
                  ...
                  t_for(g1, 0, G1); {
                    s_for(e1s, 0, E1s); {
                      ...
                      t_for(l0, 0, L0); {
                        ...
                        Weights[d5][e4][g3][d3s][e2][g1][e1s]; // read-only
                        Inputs[l5][l4][e4][g3][e2][l2][g1][e1s][l0]; // read-only
                        Outputs[l5][d5][l4][g3][d3s][l2][g1][l0]; // read-write
