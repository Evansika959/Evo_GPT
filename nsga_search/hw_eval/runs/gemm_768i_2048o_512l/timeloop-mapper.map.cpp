  ...
  static const int D5 = 128;
  static const int E4 = 48;
  ...
  Weights.Resize({ D5, E4, G3, D3s, G1, E1s });
  Inputs.Resize({ E4, G3, L2, G1, E1s, L0 });
  ...
  // DRAM tiles 
  Weights.AddTileLevel(D5*E4*G3*D3s*G1*E1s*1, D5*E4*G3*D3s*G1*E1s*1, 1, 1 * kBackingStoreLatency);
  Weights.BindCurrentTileLevel("DRAM", 1);
  Inputs.AddTileLevel(E4*G3*L2*G1*E1s*L0*1, E4*G3*L2*G1*E1s*L0*1, 1, 1 * kBackingStoreLatency);
            ...
            t_for(g1, 0, G1); {
              s_for(e1s, 0, E1s); {
                ...
                t_for(l0, 0, L0); {
                  ...
                  Weights[d5][e4][g3][d3s][g1][e1s]; // read-only
                  Inputs[e4][g3][l2][g1][e1s][l0]; // read-only
