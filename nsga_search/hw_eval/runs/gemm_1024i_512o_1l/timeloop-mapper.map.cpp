  ...
  Weights.Resize({ D5, E4, G3, D3s, E2, G1, E1s, G0 });
  Inputs.Resize({ E4, G3, E2, G1, E1s, G0 });
  Outputs.Resize({ D5, G3, D3s, G1, G0 });
  ...
  // DRAM tiles 
  Weights.AddTileLevel(D5*E4*G3*D3s*E2*G1*E1s*G0*1, D5*E4*G3*D3s*E2*G1*E1s*G0*1, 1, 1 * kBackingStoreLatency);
  Weights.BindCurrentTileLevel("DRAM", 1);
  Inputs.AddTileLevel(E4*G3*E2*G1*E1s*G0*1, E4*G3*E2*G1*E1s*G0*1, 1, 1 * kBackingStoreLatency);
  Inputs.BindCurrentTileLevel("DRAM", 1);
  Outputs.AddTileLevel(D5*G3*D3s*G1*G0*1, D5*G3*D3s*G1*G0*1, 1, 1 * kBackingStoreLatency);
  Outputs.BindCurrentTileLevel("DRAM", 1);

  t_for(d5, 0, D5); {

    // Scratchpad tiles 
    Weights.AddTileLevel(E4*G3*D3s*E2*G1*E1s*G0*1, E4*G3*D3s*E2*G1*E1s*G0*1, 1, 1 * kDRAMLatency);
    Weights.BindCurrentTileLevel("Scratchpad", 1);
    Inputs.AddTileLevel(E4*G3*E2*G1*E1s*G0*1, E4*G3*E2*G1*E1s*G0*1, 1, 1 * kDRAMLatency);
    ...
    t_for(e4, 0, E4); {
      ...
      t_for(g3, 0, G3); {
        s_for(d3s, 0, D3s); {
          ...
          Outputs.AddTileLevel(G1*G0*1, G1*G0*1, 1, 1 * kinter_PERows_spatialLatency);
            ...
            t_for(g1, 0, G1); {
              s_for(e1s, 0, E1s); {
                ...
                t_for(g0, 0, G0); {
                  ...
                  Weights[d5][e4][g3][d3s][e2][g1][e1s][g0]; // read-only
                  Inputs[e4][g3][e2][g1][e1s][g0]; // read-only
                  Outputs[d5][g3][d3s][g1][g0]; // read-write
