  ...
  static const int G5 = 1;
  static const int G4 = 1;
  static const int G3 = 1;
  static const int D3s = 16;
  static const int D2 = 14;
  static const int E2 = 32;
  static const int G1 = 1;
  static const int E1s = 16;
  static const int G0 = 1;

  Weights.Resize({ G5, G4, G3, D3s, D2, E2, G1, E1s, G0 });
  Inputs.Resize({ G5, G4, G3, E2, G1, E1s, G0 });
  Outputs.Resize({ G5, G4, G3, D3s, D2, G1, G0 });
  ...
  // DRAM tiles 
  Weights.AddTileLevel(G5*G4*G3*D3s*D2*E2*G1*E1s*G0*1, G5*G4*G3*D3s*D2*E2*G1*E1s*G0*1, 1, 1 * kBackingStoreLatency);
  Weights.BindCurrentTileLevel("DRAM", 1);
  Inputs.AddTileLevel(G5*G4*G3*E2*G1*E1s*G0*1, G5*G4*G3*E2*G1*E1s*G0*1, 1, 1 * kBackingStoreLatency);
  Inputs.BindCurrentTileLevel("DRAM", 1);
  Outputs.AddTileLevel(G5*G4*G3*D3s*D2*G1*G0*1, G5*G4*G3*D3s*D2*G1*G0*1, 1, 1 * kBackingStoreLatency);
  Outputs.BindCurrentTileLevel("DRAM", 1);

  t_for(g5, 0, G5); {

    // Scratchpad tiles 
    Weights.AddTileLevel(G4*G3*D3s*D2*E2*G1*E1s*G0*1, G4*G3*D3s*D2*E2*G1*E1s*G0*1, 1, 1 * kDRAMLatency);
    Weights.BindCurrentTileLevel("Scratchpad", 1);
    Inputs.AddTileLevel(G4*G3*E2*G1*E1s*G0*1, G4*G3*E2*G1*E1s*G0*1, 1, 1 * kDRAMLatency);
    ...
    t_for(g4, 0, G4); {
      ...
      t_for(g3, 0, G3); {
        s_for(d3s, 0, D3s); {
          ...
          Outputs.AddTileLevel(D2*G1*G0*1, D2*G1*G0*1, 1, 1 * kinter_PERows_spatialLatency);
            ...
            t_for(e2, 0, E2); {
              ...
              t_for(g1, 0, G1); {
                s_for(e1s, 0, E1s); {
                  ...
                  // Registers tiles 
                  Weights.AddTileLevel(G0*1, G0*1, 1, 1 * kinter_PECols_spatialLatency);
                  ...
                  t_for(g0, 0, G0); {
                    ...
                    Weights[g5][g4][g3][d3s][d2][e2][g1][e1s][g0]; // read-only
                    Inputs[g5][g4][g3][e2][g1][e1s][g0]; // read-only
                    Outputs[g5][g4][g3][d3s][d2][g1][g0]; // read-write
