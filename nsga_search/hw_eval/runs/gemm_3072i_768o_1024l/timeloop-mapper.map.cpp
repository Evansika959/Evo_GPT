  ...
  static const int L5 = 4;
  static const int D5 = 3;
  static const int E5 = 32;
  static const int E4 = 6;
  ...
  Weights.Resize({ D5, E5, E4, G3, D3s, D2, G1, E1s });
  Inputs.Resize({ L5, E5, E4, G3, L2, G1, E1s, L0 });
  Outputs.Resize({ L5, D5, G3, D3s, D2, L2, G1, L0 });
  ...
  // DRAM tiles 
  Weights.AddTileLevel(D5*E5*E4*G3*D3s*D2*G1*E1s*1, D5*E5*E4*G3*D3s*D2*G1*E1s*1, 1, 1 * kBackingStoreLatency);
  Weights.BindCurrentTileLevel("DRAM", 1);
  Inputs.AddTileLevel(L5*E5*E4*G3*L2*G1*E1s*L0*1, L5*E5*E4*G3*L2*G1*E1s*L0*1, 1, 1 * kBackingStoreLatency);
  Inputs.BindCurrentTileLevel("DRAM", 1);
  Outputs.AddTileLevel(L5*D5*G3*D3s*D2*L2*G1*L0*1, L5*D5*G3*D3s*D2*L2*G1*L0*1, 1, 1 * kBackingStoreLatency);
  ...
  t_for(l5, 0, L5); {
    t_for(d5, 0, D5); {
      t_for(e5, 0, E5); {
        ...
        // Scratchpad tiles 
        Weights.AddTileLevel(E4*G3*D3s*D2*G1*E1s*1, E4*G3*D3s*D2*G1*E1s*1, 1, 1 * kDRAMLatency);
        Weights.BindCurrentTileLevel("Scratchpad", 1);
        Inputs.AddTileLevel(E4*G3*L2*G1*E1s*L0*1, E4*G3*L2*G1*E1s*L0*1, 1, 1 * kDRAMLatency);
                  ...
                  t_for(g1, 0, G1); {
                    s_for(e1s, 0, E1s); {
                      ...
                      t_for(l0, 0, L0); {
                        ...
                        Weights[d5][e5][e4][g3][d3s][d2][g1][e1s]; // read-only
                        Inputs[l5][e5][e4][g3][l2][g1][e1s][l0]; // read-only
                        Outputs[l5][d5][g3][d3s][d2][l2][g1][l0]; // read-write
